...
import { Driver, DriverSeasonStats, DriverStanding, Session, SessionResult } from './types';

const API_BASE = 'https://api.openf1.org/v1';

export async function getSessions(year: number, sessionType: string = 'Race'): Promise<Session[]> {
  const res = await fetch(`${API_BASE}/sessions?year=${year}&session_type=${sessionType}`, { cache: 'no-store' });
  if (!res.ok) throw new Error('Failed to fetch sessions');
  const sessionData = await res.json();

  // Sort sessions by date
  return sessionData.sort((a: Session, b: Session) =>
...
export async function getSessionResults(sessionKey: number, retries = 3): Promise<SessionResult[]> {
  for (let i = 0; i < retries; i++) {
    try {
      const res = await fetch(`${API_BASE}/session_result?session_key=${sessionKey}`, { cache: 'no-store' });

      if (res.ok) {
        const data = await res.json();
        try { await fs.appendFile('/tmp/debug_f1.log', `SUCCESS ${sessionKey}: ${Array.isArray(data) ? data.length : 'Not Array'} items\n`); } catch(e){}
        return data;
      }
      ...
      if (res.status === 429) {
        ...
        console.warn(`Rate limit hit for session ${sessionKey}. Retrying in ${delay}ms...`);
        await new Promise(resolve => setTimeout(resolve, delay));
        continue;
      }

      // Other error statuses
      console.warn(`Failed to fetch results for session ${sessionKey}: Status ${res.status}`);
      try { await fs.appendFile('/tmp/debug_f1.log', `FAIL ${sessionKey}: Status ${res.status}\n`); } catch(e){}
      return [];
    } catch (e) {
      console.error(`Error fetching session ${sessionKey}:`, e);
      try { await fs.appendFile('/tmp/debug_f1.log', `ERROR ${sessionKey}: ${e}\n`); } catch(ex){}
...
export async function getSeasonPoints(year: number): Promise<DriverSeasonStats[]> {
  ...
  if (year === 2025) {
    try {
        ...
        console.warn('Failed to fetch fallback drivers for 2025 country backfill');
    }
  }
  ...
  drivers.forEach(d => {
    ...
    if (!d.country_code && fallbackDriverMap.has(d.driver_number)) {
        d.country_code = fallbackDriverMap.get(d.driver_number)?.country_code || "";
    }
  ...
  for (const { session, results } of historyData) {
    ...
    if (session.session_key === 9693) {
        try {
            ...
            await fs.appendFile('/tmp/debug_f1.log', `AGGREGATE 9693: Results ${results.length}. Lando found: ${!!lando}, Points: ${lando?.points}, DriverNumType: ${typeof lando?.driver_number}\n`);
        } catch(e){}
    }
      ...
      if (!stats) {
        ...
        // Check fallback map for country if available
         if (fallbackDriverMap.has(dNum)) {
             stats.driver_info = { ...stats.driver_info!, ...fallbackDriverMap.get(dNum)! };
         }
        driverStatsMap.set(dNum, stats);
      }

      // Update points
      const currentTotal = (driverCumulativePoints.get(dNum) || 0) + result.points;
  ...
  const finalStats = Array.from(driverStatsMap.values()).sort((a, b) => b.total_points - a.total_points);
  ...
  try {
    ...
    try {
        await fs.writeFile(path.join(process.cwd(), 'debug_server_error.txt'), `Error writing cache: ${JSON.stringify(error)}`);
    } catch (e) {}
  }
