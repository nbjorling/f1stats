...
type PitStopStatistic = {
  medianPitStop: number;
  fastestPitStop: Pitstop | null;
  slowestPitStop: Pitstop | null;
};
...
const getTextColor = (
  stopTime: number,
  pitStopStatistics: PitStopStatistic
) => {
  if (stopTime === pitStopStatistics?.fastestPitStop?.pit_duration) {
    return 'text-purple-500';
  }
  if (
    stopTime > (pitStopStatistics?.medianPitStop || 0) &&
    stopTime < (pitStopStatistics?.medianPitStop || 0) + 1
  ) {
    return 'text-green-500';
  }

  if (stopTime === (pitStopStatistics.slowestPitStop || 0)) {
    return 'text-red-500';
  }
...
const Legend = () => (
  <Card>
    <CardContent>
      <div>
        <span className="bg-purple-500 w-3 h-3 inline-block mr-2" />
        Fastest
      </div>
      <div>
        <span className="bg-green-500 w-3 h-3 inline-block  mr-2" />
        Within 1 second of median
      </div>
      <div>
        <span className="bg-red-500 w-3 h-3 inline-block  mr-2" />
        Slowest
      </div>
    </CardContent>
  </Card>
  ...
  <Card>
    <CardContent>
      <div className="flex items-center gap-2">
        ...
        <span className="text-lg font-semibold">Pit Stop Statistics</span>
      </div>
      <ul className="mt-2">
        <li>
          <strong>Median:</strong> {pitStopStatistics.medianPitStop}s
        </li>
        <li>
          <strong>Fastest:</strong>{' '}
          {pitStopStatistics.fastestPitStop
            ? `${pitStopStatistics.fastestPitStop.pit_duration}s`
            : 'N/A'}
        </li>
        <li>
          <strong>Slowest:</strong>{' '}
          {pitStopStatistics.slowestPitStop
            ? `${pitStopStatistics.slowestPitStop.pit_duration}s`
            : 'N/A'}
        </li>
      </ul>
    </CardContent>
...
export default function PitStops() {
  ...
  const [pitStopStatistics, setPitStopStatistics] = useState<PitStopStatistic>({
    medianPitStop: 0,
    fastestPitStop: null,
    slowestPitStop: null,
  });
  ...
  useEffect(() => {
    async function fetchPitStops() {
      ...
      try {
        ...
        setPitStopStatistics({
          medianPitStop: medianPitStop,
          fastestPitStop: pitStopsData.reduce((prev, current) => {
            return prev.pit_duration < current.pit_duration ? prev : current;
          }),
          slowestPitStop: pitStopsData.reduce((prev, current) => {
            return prev.pit_duration > current.pit_duration ? prev : current;
          }),
        });
        ...
        const drivers = Object.entries(driverMap).map(
          ([driver_number, stops]) => {
            ...
            return {
              ...
              name_acronym: thisDriver.name_acronym,
              stops,
            };
          }
        );
    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 grid gap-4">
          <Legend />
          <PitStopStatistics pitStopStatistics={pitStopStatistics} />
          {pitStops.map((driver) => (
